...
def root():
  ...
  global operator
  textin.set(sqrt(int(operator)))

def power():
  global operator
  textin.set(int(operator) ** 2)
...
powers=Button(me,padx=14,pady=14,bd=4,bg='white',command=lambda:power(),text="^",font=("Courier New",16,'bold'))
powers.place(x=75, y=310)

sqrt=Button(me,padx=14,pady=14,bd=4,bg='white',command=lambda:root(),text="√",font=("Courier New",16,'bold'))
sqrt.place(x=140,y=310)
